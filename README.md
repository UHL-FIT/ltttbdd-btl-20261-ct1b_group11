[![Review Assignment Due Date](https://classroom.github.com/assets/deadline-readme-button-22041afd0340ce965d47ae6ef1cefeee28c7c493a6346c4f15d667ab976d596c.svg)](https://classroom.github.com/a/H8V-fdC0)
[![Open in Visual Studio Code](https://classroom.github.com/assets/open-in-vscode-2e0aaae1b6195c2367325f4f02e2d04e9abb55f0b24a779b69b11b9e10269abc.svg)](https://classroom.github.com/online_ide?assignment_repo_id=24142700&assignment_repo_type=AssignmentRepo)
## Bài tập lớn Lập trình trên thiết bị di động
 BuddyBudget - Ứng dụng Quản lý Tài chính Cá nhân

## 1. Thông tin Đề tài & Thành viên thực hiện
- **Tên đề tài:** Xây dựng ứng dụng quản lý chi tiêu cá nhân trên Android.
- **Thành viên thực hiện:**
    1. Nguyễn Minh Đức
    2. Vũ Nguyễn Đức Hải
    3. Đỗ Nguyễn Hải Đăng
    4. Phạm Nguyễn Minh Hoàng
- **Lớp:** CNTT K1B
- **Giảng viên hướng dẫn:** ThS.Vũ Duy Sơn

---

## 2. Mô tả ứng dụng
**BuddyBudget** là công cụ hỗ trợ người dùng kiểm soát tài chính cá nhân một cách thông minh và tiện lợi. Ứng dụng tập trung vào các tính năng cốt lõi:
- **Quản lý đa ví:** Theo dõi số dư từ nhiều nguồn khác nhau .
- **Ghi chép giao dịch:** Thu thập lịch sử thu nhập và chi tiêu hàng ngày.
- **Phân loại thông minh:** Sắp xếp các khoản chi theo danh mục (ăn uống, di chuyển, giải trí...) để dễ dàng theo dõi.
- **Thống kê trực quan:** Cung cấp biểu đồ tổng quan giúp người dùng nhận biết thói quen tiêu dùng và điều chỉnh kế hoạch tài chính kịp thời.
- **Tìm kiếm & Lọc:** Hỗ trợ truy xuất nhanh các giao dịch cũ theo ghi chú và lọc theo tháng và ví.
---

## 📥 DOWNLOAD APK
> ### **[➔ NHẤN VÀO ĐÂY ĐỂ TẢI BẢN APK CÀI ĐẶT](https://github.com/UHL-FIT/ltttbdd-btl-20261-ct1b_group11/blob/main/budgetbuddy.apk)**
> *Dung lượng: ~21MB | Phiên bản: v1.0.0*
